...
@register(
    "astrbot_plugin_chatsummary_bark",
    "Battery-rar",
    "一个基于LLM的自动历史聊天记录总结插件",
    "1.0.0",
)
...
class ChatSummary_bark(Star):
    ...
    async def summary(self, event: AstrMessageEvent):
        ...
        for msg in messages:
                ...
                if part["type"] == "text":
                    ...
                    try:
                        ...
                        if "desc" in json_content.get("meta", {}).get("news", {}):
                            message_text += (
                                f"[分享内容]{json_content['meta']['news']['desc']} "
                            )
            ...
            at_all = any(
                seg.get("type") == "at"
                and str(seg.get("data", {}).get("qq", "")) == "all"
                for m in messages
            ...
            at_me = any(
                seg.get("type") == "at"
                and str(seg.get("data", {}).get("qq", "")) == str(myid)
                for m in messages
                for seg in m.get("message", [])
            )
        ...
        llm_response = await self.context.get_using_provider().text_chat(
            prompt=load_prompt(),
            contexts=[{"role": "user", "content": str(msg)}],
        )
        # 判空：None / "NULL" / "" / 仅空白
        ...
        async def push_bark_encrypted(Name: str, text: str) -> None:
            # 1) 明文载荷
            payload = {"body": text, "title": Name, "icon": icon_url}
            plaintext = json.dumps(
                payload, ensure_ascii=False, separators=(",", ":")
            ).encode("utf-8")

            # 2) 加密（固定 IV + ASCII Key）
            ciphertext_b64 = _aes256_cbc_encrypt_b64(
                plaintext, BARK_CIPHER_KEY.encode("utf-8"), BARK_IV
            )
